...
The directory is flat on purpose: every file is an experiment entry point, invoked
from the repository root as `python scripts/<name>.py`, and several import each other
as sibling modules. That makes the directory large, so this index says what is current,
what is a standalone tool, and what is kept as the record of a closed line of work.
...
Run by hand on dumped activations; they decide which variants earn a GPU run and never
produce conclusions on their own.
...
These are from the first phase of the project, when the beds were Dream, SciQ, and PAWS
and the model pair was cross-family. They still parse and their imports resolve, but
they are not part of the current pipeline and their results have been superseded. They
stay because the paper reports what was tried and did not work, and those numbers came
from here.
