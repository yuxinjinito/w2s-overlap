...
# Fast SciQ prompt-ablation smoke test.
#
# Goal:
#   Check whether switching SciQ from the support-included prompt to the
#   original datacentric_w2s no-support prompt reduces the weak/kNN saturation.
#
# This intentionally runs no LoRA training. It still extracts weak/strong
...
OUTPUT_DIR="${OUTPUT_DIR:-results/sciq_no_support_prompt_smoke_0604}"
...
echo "=== SciQ no-support prompt smoke ==="
echo "OUTPUT_DIR=${OUTPUT_DIR}"
echo "N_TRAIN=${N_TRAIN}"
echo "N_VAL=${N_VAL}"
echo "N_TEST=${N_TEST}"
echo "SEED=${SEED}"
echo "Prompt format: Q: {question} A: {candidate}"
echo "LoRA runs: none"
